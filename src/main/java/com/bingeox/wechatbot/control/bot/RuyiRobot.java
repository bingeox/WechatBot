...
public class RuyiRobot implements Robot {
    ...
    public String getMessage(String text) {
        ...
        BotResult<RuyiResult> botResult = resp.toJavaObject(new TypeReference<BotResult<RuyiResult>>() {
        });
        String answer = "搜噶";
        if (botResult.getCode() == Constants.ZERO || botResult.getCode() == Constants.TWO_HUNDRED){
            List<RuyiResult.Intent.Output> outputs = botResult.getResult().getIntents().get(0).getOutputs();
